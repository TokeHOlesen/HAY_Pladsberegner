...
class TruckView(QWidget):
    ...
    def update_standard_pallet_width(self) -> None:
        ...
        """
        pallet_width: int = self.width() - 4 * self.gutter // 3
        while pallet_width > 0:
            truck_rect_width: int = 3 * pallet_width + 4 * self.gutter
            truck_rect_height: int = int(round(truck_rect_width * self.aspect))
            if truck_rect_width <= self.width() and truck_rect_height <= self.height():
                break
            pallet_width -= 1
        self.standard_pallet_width = pallet_width

    def update_border_rect(self) -> None:
        """
        ...
        """
        width: int = 3 * self.standard_pallet_width + 4 * self.gutter
        height: int = int(round(width * self.aspect))
        self.border_rect = QRect(0, 0, width, height)

    def update_pallet_rect_dimensions(self) -> None:
        """
        ...
        self.loose_pallet_dimensions[60] = [self.pallet_rect_dimensions["standard_width"],
                                            self.pallet_rect_dimensions["v-60-height"]]
        self.loose_pallet_dimensions[120] = [self.pallet_rect_dimensions["standard_width"],
                                             self.pallet_rect_dimensions["v-120-height"]]
        self.loose_pallet_dimensions[145] = [self.pallet_rect_dimensions["standard_width"],
                                             self.pallet_rect_dimensions["v-145-height"]]
        self.loose_pallet_dimensions[130] = [self.pallet_rect_dimensions["v-130-width"],
                                             self.pallet_rect_dimensions["v-130-height"]]
        self.loose_pallet_dimensions[17080] = [self.pallet_rect_dimensions["standard_width"],
                                               self.pallet_rect_dimensions["v-17080-height"]]
        self.loose_pallet_dimensions[17090] = [self.pallet_rect_dimensions["v-17090-width"],
                                               self.pallet_rect_dimensions["v-17090-height"]]

    def generate_arrangement(self, arrangement) -> None:
        """
        ...
        x: int = self.border_rect.left() + self.gutter
        y: int = self.border_rect.top() + self.gutter + self.brush_offset
    ...
    def generate_loose_pallet(self, pallet_type, left_margin, text_margin, spacing, font_height, count) -> None:
        width, height = self.loose_pallet_dimensions[pallet_type]
        color = PALLET_COLORS[str(pallet_type)]
        self.pallet_rects.append(PalletRect(left_margin, self.brush_offset, width, height, color=color))
        self.text_to_draw.append([text_margin, self.brush_offset + height // 2 + font_height // 3, f"x{count}"])
        self.brush_offset += height + spacing

    def draw_border_rect(self, painter, border_thickness) -> None:
    ...
    def draw_pallet_rects(self, painter, border_thickness) -> None:
    ...
    def draw_truck(self, painter, width, height, to_print=False) -> None:
        """
    ...
    def draw_loose_pallets(self, painter, width, height, to_print=False) -> None:
        painter.setRenderHint(QPainter.RenderHint.Antialiasing, False)
        self.resize(width, height)
        ...
        for pallet_type in leftover_count:
            self.generate_loose_pallet(pallet_type,
                                       left_margin,
                                       text_margin,
                                       spacing,
                                       font_height,
